# C-programming

This Repository contains collection of different C programs.

## Programs List

- [Sorting](./programs/sorting/)
    - [Bubble Sort](./programs/sorting/bubblesort.c)
    - [Selection Sort](./programs/sorting/selectionsort.c)
    - [Insertion Sort](./programs/sorting/Insertion_sort.c)
    - [Quick Sort](./programs/sorting/Quick_sort.c)
    - [Merge Sort](./programs/sorting/Merge_sort.c)

- [Searching](./programs/searching/)
    - [Linear Search](./programs/searching/linearsearch.c)
    - [Binary Search](./programs/searching/binarysearch.c)
   
- [Algorithms](./programs/Algorithms/)
      - [Warshel's Algo](./programs/Algorithms/warshells_algo.c)
      - [Topological Order algo](./programs/Algorithms/topological_order.c)
      - [Dijkstra's Algo](./programs/Algorithms/Dijkstra_algo.c)
      - [Minimum spanning tree algo](./programs/Algorithms/Minimum_spanning_tree.c)
      - [Prim's algo](./programs/Algorithms/Prim's algo.c)
- [Other Programs](./programs/others/)
   - [Multiplication of 2 numbers](./programs/others/2_number_multiplication.c)
   - [Multiplication of n numbers](./programs/others/multiplication_of_n_number.c)
   - [Sum of fist n natural number](./programs/others/Sum_of_first_n_naturalno.c)
